...
| 카테고리 | 알고리즘 | 방식 | 특징 및 장단점 |
| :--- | :--- | :--- | :--- |
| **Batch** (처리량 중심) | **FCFS** | 비선점 | 도착 순서대로 처리. 단순하지만 **호위 효과(Convoy Effect)** 발생 가능 |
| | **SJF** | 비선점 | 실행 시간이 짧은 순서대로 처리. 평균 대기 시간 최소화 |
| | **SRTN** | 선점 | 남은 실행 시간이 가장 짧은 프로세스 우선 처리 (SJF의 선점형 버전) |
| **Interactive** (응답성 중심) | **Round-Robin** | 선점 | 동일한 **Time Quantum** 부여. 응답성이 좋으나 오버헤드 주의 |
| | **Priority** | 선점/비선점 | 우선순위 기반. **기아 현상(Starvation)** 해결을 위해 Aging 기법 필요 |
| **Real-time** (마감기한 중심) | **EDF** | 유동적 | 마감 시간(Deadline)이 가장 가까운 작업부터 실행 |
| | **RMS** | 선점 | 주기(Period)가 짧을수록 높은 우선순위를 부여하는 정적 방식 |
